...
class SimulationApp:
    ...
    def open_modify_od_window(self):
        """Open a window to apply a modification (percentage or value) to selected origin and destination zones in the OD matrix."""
        if not self.od_matrix_file:
        ...
        tk.Label(modify_window, text="Select Adjustment Type:").pack(pady=5)
        tk.Radiobutton(modify_window, text="By Percentage", variable=adjustment_type_var, value="percentage").pack(anchor=tk.W)
        tk.Radiobutton(modify_window, text="By Value", variable=adjustment_type_var, value="value").pack(anchor=tk.W)

        # Entry for Percentage or Value
        tk.Label(modify_window, text="Enter amount to modify the matrix:").pack(pady=5)
        adjustment_entry = tk.Entry(modify_window)
        ...
        all_zones = sorted(set([row[0] for row in od_data] + [row[1] for row in od_data]))  # Get all unique zones

        # Populate listboxes with all unique zones
        for zone in all_zones:
            from_zones_listbox.insert(tk.END, zone)
            to_zones_listbox.insert(tk.END, zone)

        def apply_modification():
            adjustment_amount = adjustment_entry.get().strip()
            operation = operation_var.get()
            adjustment_type = adjustment_type_var.get()

            # Get selected zones from listboxes
            from_zone_indices = from_zones_listbox.curselection()
            to_zone_indices = to_zones_listbox.curselection()

            # Ensure at least one origin and one destination zone is selected
            if not from_zone_indices:
                messagebox.showerror("Error", "Please select at least one origin zone.")
                return
            if not to_zone_indices:
                messagebox.showerror("Error", "Please select at least one destination zone.")
                return

            # Retrieve the selected zones from the listboxes
            from_zones = [from_zones_listbox.get(i) for i in from_zone_indices]
            to_zones = [to_zones_listbox.get(i) for i in to_zone_indices]

            # Apply the modification based on selected adjustment type
            try:
                if adjustment_type == "percentage":
                    percentage = float(adjustment_amount)
                    if percentage < 0:
                        raise ValueError("Percentage cannot be negative.")
                    header_lines, updated_od_data = self.data_processor.modify_od_matrix_percentage(
                        self.od_matrix_file, percentage, operation, from_zones, to_zones, self.file_handler
                    )
                elif adjustment_type == "value":
                    value = int(adjustment_amount)
                    header_lines, updated_od_data = self.data_processor.modify_od_matrix_value(
                        self.od_matrix_file, value, operation, from_zones, to_zones, self.file_handler
                    )
                else:
                    messagebox.showerror("Error", "Invalid adjustment type selected.")
                    return
                ...
                if save_as_new:
                    ...
                    self.file_handler.write_od_matrix(self.od_matrix_file, header_lines, updated_od_data)
                    self.output_text.insert(tk.END, f"OD Matrix modified and saved: {operation} {adjustment_amount} ({adjustment_type}) for selected zones.\n")
...
class FileHandler:
    def __init__(self):
        ...
        self.edges = {}  # Initialize self.edges here
        self.edge_map = {}
    ...
    def parse_edges_from_network_file(self, network_file):
        """Parse edges where 'from' and 'to' refer to other edge IDs."""
        edges = {}
        try:
            ...
            for edge in root.findall('edge'):
                edge_id = edge.attrib.get('id', None)
                edge_name = edge.attrib.get('name', 'Unknown')
                from_edge = edge.attrib.get('from', None)  # Now refers to another edge ID
                to_edge = edge.attrib.get('to', None)  # Now refers to another edge ID

                if edge_id is None or from_edge is None or to_edge is None:
                    continue  # Skip edges with missing attributes

                # Store edge data with 'from' and 'to' referring to other edges
                edge_data = {
                    'id': edge_id,
                    'from': from_edge,
                    'to': to_edge,
                ...
                # Store edges by name for easy access
                if edge_name not in edges:
                    edges[edge_name] = []
                edges[edge_name].append(edge_id)

            self.edge_map = edges  # Store edges by name
            return edges
        except Exception as e:
            print(f"Error parsing network file: {str(e)}")
...
class DataProcessor:
    ...
    def modify_od_matrix_percentage(self, od_matrix_file, percentage, operation, from_zones, to_zones, file_handler):
        """Modify the OD matrix by adding or subtracting a percentage for selected origin and destination zones."""
        header_lines, od_data = file_handler.read_od_matrix(od_matrix_file)

        # Convert OD data to dictionary for easy manipulation
        od_matrix = {(row[0], row[1]): row[2] for row in od_data}
        ...
        # Loop through selected origin and destination zones
        for from_zone in from_zones:
            for to_zone in to_zones:
                # Ensure the OD pair exists
                if (from_zone, to_zone) in od_matrix:
                    trips = od_matrix[(from_zone, to_zone)]
                    if operation == 'add':
                ...
                #else:
                    #messagebox.showwarning("Warning", f"OD pair from {from_zone} to {to_zone} does not exist in the matrix.")
        
        # Convert the modified dictionary back to a list format for saving
        updated_od_data = [[key[0], key[1], value] for key, value in od_matrix.items()]

        return header_lines, updated_od_data

    def modify_od_matrix_value(self, od_matrix_file, value, operation, from_zones, to_zones, file_handler):
        """Modify the OD matrix by adding or subtracting a specific value for selected origin and destination zones."""
        header_lines, od_data = file_handler.read_od_matrix(od_matrix_file)

        # Convert OD data to a dictionary for easier manipulation
        od_matrix = {(int(row[0]), int(row[1])): int(row[2]) for row in od_data}

        if operation not in ['add', 'subtract']:
            messagebox.showerror("Error", "Invalid operation. Use 'add' or 'subtract'.")
            return

        # Loop through each selected origin and destination zone pair
        for from_zone in from_zones:
            for to_zone in to_zones:
                # Ensure the OD pair exists in the matrix
                if (from_zone, to_zone) in od_matrix:
                    current_trips = od_matrix[(from_zone, to_zone)]
                    if operation == 'add':
                        # Add the specified value to the existing trips
                        od_matrix[(from_zone, to_zone)] = current_trips + value
                    elif operation == 'subtract':
                        # Subtract the specified value, ensuring no negative values
                        new_value = current_trips - value
                        if new_value < 0:
                            messagebox.showerror("Error", f"Resulting trips from {from_zone} to {to_zone} cannot be negative.")
                            return
                        od_matrix[(from_zone, to_zone)] = new_value

        # Convert the modified dictionary back to list format for saving
        updated_od_data = [[key[0], key[1], value] for key, value in od_matrix.items()]
